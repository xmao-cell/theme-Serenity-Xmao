...
- 当前仓库：[xmao-cell/theme-Serenity-Xmao](https://github.com/xmao-cell/theme-Serenity-Xmao)
...
- 优化文章详情页移动端顶部信息布局，改善长标题、作者、日期、评论、阅读量及分类标签在小屏幕下的排列。
- 将文章封面与正文卡片统一为 `12px` 圆角，并使移动端封面与正文保持一致的左右留白。
- 增加可选的移动端抽屉导航，默认仍使用原来的导航样式。
- 替换主题自带的亮色、暗色首页背景，并调整导航按钮的颜色和反馈。
...
| 移动端导航 | 可在原有导航和抽屉导航之间切换，菜单内容读取 Halo 主菜单 |
...
<table>
  ...
  <tr>
    ...
    <td>封面 Hero（与正文统一圆角，移动端自适应信息布局）· 浮动目录导航（自适应高度）· 阅读进度条 · 相关文章推荐 · 版权声明 · 社交分享 · 打赏组件 · 背景壁纸</td>
  </tr>
  <tr>
...
| 配置分组 | 可配置项 |
|:---------|:---------|
| 基本设置 | 站点标题 · 作者 · 描述 · Logo · Favicon · 默认主题模式 · **亮色 / 暗色主题色** · **移动端导航样式开关** · 导航图标 · 分类图标 · 自定义 Head/Script 代码注入 |
...
> 📦 部分插件均可在 [Halo 应用市场](https://www.halo.run/store/apps) 中安装。
...
> 已安装 `2.1.4` 的用户可以直接上传新版压缩包覆盖。新版移动端导航默认关闭，可在「主题设置 → 基本设置」中开启。
